...
d3.json("http://localhost:1990/api/v1/log/20", function (data) {
            // format the data a bit
            var numberFormat = d3.format(",f");
            var dateformat=d3.time.format("dd.mm.yyyy hh:MM:ss").parse;            
            data.forEach(function(d) {
            	var dateTime = new Date(parseInt(d.timestamp_1));
         //   	d.year_month_day_hour=dateformat(dateTime);
            });
            ...
            var expensesByCampusGroup = expensesByCampus.group().reduceSum(function(d) { return d.cnt; });


            var expensesByYear = ndx.dimension(function (d) {
                return d.year_month_day_hour;
            });

            var expensesByYearGroup = expensesByYear.group().reduceSum(function (d) { return d.cnt; });
